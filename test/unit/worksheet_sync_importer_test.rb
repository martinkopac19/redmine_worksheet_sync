...
class WorksheetSyncImporterTest < ActiveSupport::TestCase
  ...
  def test_imports_tag_with_comment
    ...
    assert_difference 'TimeEntry.count', 1 do
      rep = import([entry(id: 1001, title: '#1 Fix login')])
    end
  ...
  def test_imports_bare_tag_with_empty_comment
    assert_difference 'TimeEntry.count', 1 do
      import([entry(id: 1002, title: '#1')])
    end
    assert_equal '', TimeEntry.order(:id).last.comments.to_s
  end

  def test_is_idempotent
    import([entry(id: 1003, title: '#1 once')])
    assert_no_difference 'TimeEntry.count' do
      rep = import([entry(id: 1003, title: '#1 once')])
  ...
  def test_skips_excluded_work_type
    assert_no_difference 'TimeEntry.count' do
      rep = import([entry(id: 1005, title: '#1 x', work_type: 'Sick Leave')])
  ...
  def test_skips_missing_issue
    assert_no_difference 'TimeEntry.count' do
      rep = import([entry(id: 1006, title: '#999999 ghost')])
  ...
  def test_dry_run_writes_nothing
    assert_no_difference 'TimeEntry.count' do
      rep = import([entry(id: 1007, title: '#1 preview')], dry_run: true)
      assert_equal 1, rep[:imported].size
    end
  end
  ...
  def test_cron_lock_check_skips_unlocked_days
    ...
    today = Date.today
    assert_no_difference 'TimeEntry.count' do
      rep = WorksheetSync::Importer.new(settings, FakeClient.new([entry(id: 1008, title: '#1 x', date: today.to_s)]))
